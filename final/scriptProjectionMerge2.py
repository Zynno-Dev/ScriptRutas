...
if "NQN" in UGs:
    ...
    for fc_path in feature_classes:
        projected_fc_name = f"{os.path.splitext(os.path.basename(fc_path))[0]}"
...
if "ACA" in UGs:
    gdb_path = os.path.join(carpeta_actual, 'Servicios_ACA.gdb')

    proyectado_gdb_path = os.path.join(carpeta_actual, 'proyectado3.gdb')
...
if "NQN" in UGs and "GSJ" in UGs and "ACA" in UGs:
    ...
    if "NQN" in UGs:
        ...
        for fc in lista_fc_geodatabase3:
            ...
            arcpy.CopyFeatures_management(geodatabase3 + '\\' + fc, geodatabase_salida + '\\' + nombre_fc)
...
try:
    ...
    log("No se pudo eliminar la geodatabase proyectado.gdb")
try:
    rmtree(os.path.join(carpeta_actual, 'proyectado2.gdb'))
except:
    log("No se pudo eliminar la geodatabase proyectado2.gdb")
try:
    rmtree(os.path.join(carpeta_actual, 'proyectado3.gdb'))
except:
    log("No se pudo eliminar la geodatabase proyectado3.gdb")